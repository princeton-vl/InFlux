...
bool PinholeProjection<DISTORTION_T>::initializeIntrinsics(const std::vector<GridCalibrationTargetObservation> &observations) {
  ...
  const char* focal_length_guess_str = std::getenv("FOCAL_LENGTH_GUESS");

  if(focal_length_guess_str != nullptr) {
    // Attempt to convert to float

    try {
      float focal_length_guess = std::stof(focal_length_guess_str);
      std::cout << std::fixed << std::setprecision(5);
      std::cout << "DETECTED: " << focal_length_guess << std::endl;
      if(std::isfinite(focal_length_guess))
        f_guesses.push_back(focal_length_guess);
    ...
    std::cout << "DETECTED: NO GUESS!" << std::endl;
  ...
  if(f_guesses.empty()) {
    ...
    if(manual_input != nullptr) {
      double input_guess = 10000.;
      std::cout << "Initialization of focal length failed. Using hardcoded guess of 10000." << std::endl;
      // std::cin >> input_guess;
