...
class ImageDatasetReaderIterator(object):
...
class FramesImageDatasetReader(object):
  def __init__(self, frames_dir, from_to=None):
    self.frames_dir = frames_dir
    self.topic = None
    self.first_time = acv.Time.now()
    self.filenames = [f for f in os.listdir(self.frames_dir) if os.path.isfile(os.path.join(self.frames_dir, f))]
    self.filenames = [f for f in self.filenames if os.path.splitext(f)[1].lower() in ['.bmp', '.png', '.jpg', '.jpeg', '.tiff', '.tif']]
    self.filenames = sorted(self.filenames)  # Important for indexing purposes
    assert len(self.filenames) > 0, "No image files in frames directory!"

    if from_to:
      assert from_to[1] <= len(self.filenames), f"Invalid from-to range - there are only {len(self.filenames)} total frames"
      self.indices = np.arange(from_to[0], from_to[1]+1)
    else:
  ...
  def readDataset(self):
    return ImageDatasetReaderIterator(self, self.indices)

  def readDatasetShuffle(self):
    indices = self.indices
    np.random.shuffle(indices)
    return ImageDatasetReaderIterator(self, indices)

  def numImages(self):
    return len(self.indices)

  def getImage(self, idx):
    img_data = cv2.imread(os.path.join(self.frames_dir, self.filenames[idx]), cv2.IMREAD_GRAYSCALE)
    timestamp = self.first_time + acv.Duration(float(idx)) # mock the timestamp
