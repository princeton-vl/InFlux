...
def multicoreExtractionWrapper(detector, taskq, resultq, dataset, clearImages, noTransformation, include_nones_on_fail):
    while 1:
        try:
        ...
        image = task[2]
        if not image:
            stamp, image = dataset.getImage(dataset.indices[idx])
        ...
        if success:
            resultq.put( (obs, idx) )
        elif include_nones_on_fail:
            resultq.put( (None, idx) )

def extractCornersFromDataset(dataset, detector, multithreading=False, numProcesses=None, clearImages=True, noTransformation=False, include_nones_on_fail=False):
    print(f"Extracting calibration target corners (multithreading={multithreading})")
    targetObservations = []
    ...
    if multithreading:   
        ...
        try:      
            ...
            if hasattr(dataset, 'frames_dir'):
                print("Frames dataset detected, lazy loadng images in parallel for speedup")
                for idx in range(dataset.numImages()):
                    taskq.put( (idx, None, None) )
            else:
            ...
            for pidx in range(0, numProcesses):
                ...
                p = multiprocessing.Process(target=multicoreExtractionWrapper, args=(detector_copy, taskq, resultq, dataset, clearImages, noTransformation, include_nones_on_fail, ))
                p.start()
                plist.append(p)
    ...
    #single threaded implementation
        ...
        for timestamp, image in dataset.readDataset():
            ...
            if success == 1:
                ...
                targetObservations.append(None)
        
            iProgress.sample()

    if len(targetObservations) == 0 or all([o is None for o in targetObservations]):
        print("\r")
