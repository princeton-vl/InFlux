...
def calibrateIntrinsics(cam_geometry, obslist, distortionActive=True, intrinsicsActive=True, override=False, init_intrinsics=[], init_distortion=[]):
    #verbose output
    if sm.getLoggingLevel()==sm.LoggingLevel.Debug:
    ...
    print(f'Before running calibrateIntrinsics: \n\tProjection{cam_geometry.geometry.projection().getParameters().flatten()}, \n\tDistortion: {cam_geometry.geometry.projection().distortion().getParameters().flatten()}')
    ...
    if override:
        ### TODO: check if we can expedite overriding without doing bundle adjustment first ###
        assert len(init_intrinsics) == 4
        assert len(init_distortion) in [0, 4]  ### TODO: update for different options ###
        
        if len(init_distortion) == 0:
            init_distortion = [0., 0., 0., 0.]

        cam_geometry.geometry.projection().setParameters(np.array(init_intrinsics)[..., None])
        cam_geometry.geometry.projection().distortion().setParameters(np.array(init_distortion)[..., None])
        print('!!Overriden!!')
