...
class CameraGeometry(object):
    ...
    def initGeometryFromObservations(self, observations, init_mode='fixed_point', init_intrinsics=[], init_distortion=[]):
        ...
        starting_intrinsics = self.geometry.projection().getParameters().flatten() # before even calibrating for the first time
        ...
        print(f'Initialiation mode: {init_mode}')

        if init_mode == 'default':
            #optimize for intrinsics & distortion
            success = kcc.calibrateIntrinsics(self, observations, override=False)
        elif init_mode == 'override':
            #optimize for intrinsics & distortion
            success = kcc.calibrateIntrinsics(self, observations, override=True, init_intrinsics=init_intrinsics, init_distortion=init_distortion)
        elif init_mode == 'run_twice':
            # Run optimization without distortion (to hopefully preserve principle point)

            # NOTE: actually principle point still gets mangled, but the
            # focal length gets closer
            success_a = kcc.calibrateIntrinsics(self, observations, distortionActive=False)

            # Run optimization a second time, but with distoriton on now
            success = kcc.calibrateIntrinsics(self, observations, override=False)
        elif init_mode == 'fixed_point':
            orig_proj = self.geometry.projection().getParameters().flatten()
            orig_cx = orig_proj[2]
            orig_cy = orig_proj[3]

            total_no_distortion_iters = 3
            total_distortion_iters = 3
            num_iters = 0

            # Run optimization, retain only the focal length, and then repeat
            while True:
                success = kcc.calibrateIntrinsics(self, observations, override=False)
                ...
                if num_iters < total_no_distortion_iters:
                    # Reset principle point and distortion
                    proj = self.geometry.projection().getParameters().flatten()
                    avg_focal_length = (proj[0] + proj[1]) / 2

                    self.geometry.projection().setParameters(np.array([[avg_focal_length], [avg_focal_length], [orig_cx], [orig_cy]]))
                    self.geometry.projection().distortion().setParameters(np.array([[0.], [0.], [0.], [0.]]))
                    num_iters += 1
                else:
                    break

            num_iters = 0
            # Run optimization, retain only the focal length and distortion, and then repeat
            while True:
                success = kcc.calibrateIntrinsics(self, observations, override=False)
                ...
                if num_iters < total_distortion_iters:
                    # Reset principle point
                    proj = self.geometry.projection().getParameters().flatten()
                    avg_focal_length = (proj[0] + proj[1]) / 2
            ...
            # Reset principle point one last time before submitting initialization
            proj = self.geometry.projection().getParameters().flatten()
            avg_focal_length = (proj[0] + proj[1]) / 2
            self.geometry.projection().setParameters(np.array([[avg_focal_length], [avg_focal_length], [orig_cx], [orig_cy]]))
        else:
        ...
        return success, starting_intrinsics
    ...
    def __init__(self, targetConfig, cameraGeometry, showCorners=False, showReproj=False, showOneStep=False):
        ...
        if targetType == 'checkerboard':
            ...
            self.grid = acv_april.GridCalibrationTargetAprilgrid(targetParams['tagRows'], 
                                                                 ...
                                                                 options)
        elif targetType == 'pointcloud':
            self.grid = acv.GridCalibrationTargetPointCloud(np.array(targetParams['points']))
        else:
...
class CameraCalibration(object):
    ...
    def addTargetView(self, timestamp, rig_observations, T_tc_guess, force=False, eval_mode=False):
