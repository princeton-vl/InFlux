...
# fix x-server patch, following https://stackoverflow.com/questions/19518352/tkinter-tclerror-couldnt-connect-to-display-localhost18-0
import matplotlib
matplotlib.use('pdf')
...
def getReprojectionErrorStatistics(all_rerrs):
    ...
    epe = (rerr_matrix[:, 0] ** 2 + rerr_matrix[:, 1] ** 2) ** 0.5
    avg_epe = epe.mean()
    max_epe = epe.max()
    # TODO: consider just saving distribution info / caching it
 
    return mean, std, avg_epe, max_epe
...
def generateReport(cself, starting_intrinsics, filename="report.pdf", showOnScreen=True, graph=None, removedOutlierCorners=None):
    ...
    sstream = StringIO()
    printParameters(cself, sstream, starting_intrinsics=starting_intrinsics)
...
def generateSmallReport(cself, starting_intrinsics, filename="report.pdf", showOnScreen=True, graph=None, removedOutlierCorners=None):
...
def saveResultTxt(cself, starting_intrinsics, filename="camera_calibration_result.txt"):
    f1=open(filename, 'w')
    printParameters(cself, f1, starting_intrinsics=starting_intrinsics)

def printParameters(cself, dest=sys.stdout, starting_intrinsics=None):
    ...
    for cidx, cam in enumerate(cself.cameras):
        ...
        if len(rerrs)>0:
            me, se, avg_epe, max_epe = getReprojectionErrorStatistics(rerrs)
            try:
              print("    reprojection error: [%f, %f] +- [%f, %f]; Avg EPE: %f; Max EPE: %f" % (me[0], me[1], se[0], se[1], avg_epe, max_epe), file=dest)
            except:
    ...
    if starting_intrinsics:
        print("", file=dest)
        print("", file=dest)
        print("Starting intrinsics", file=dest)
        print("====================", file=dest)
        for cam_intrinsics in starting_intrinsics:
            print(str(cam_intrinsics), file=dest)
        print("", file=dest)
