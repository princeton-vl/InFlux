...
DataPoint = NamedTuple(
    ...
    [
        ...
        ("frame_idx", int | None),
        ("video_id", str | None),
        ("map_group_name", str | None),
        ("map_h5_path", str | None),
    ],
)
...
def load_h5_influx_synth_format(h5_file: Path, img_root: Path) -> list[DataPoint]:
    with h5py.File(h5_file, "r", libver="latest") as f:
        datapoints = [
            DataPoint(
                name=name,
                file_name=str(group.attrs["image_path"]),
                h=int(group.attrs["h"]),
                w=int(group.attrs["w"]),
                cam_id=group.attrs["cam_id"],
                params=group.attrs["params"].astype(np.float32),
                frame_idx = str(group.attrs["frame_idx"]),
                video_id = str(group.attrs["video_id"]),
                map_group_name = name,
                map_h5_path = str(h5_file),
            )
            for name, group in f.items()
        ]
    return datapoints
...
class InFluxSynthDataset(BaseDataset):
    ...
    default_conf = {
        ...
        "produce_rays": False,  # if False, the dataset will not produce rays and ray masks for datapoints
...
class _SimpleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, conf: DictConfig, split: str):
        ...
        self._dist_map_h5_file: h5py.File | None = None
        ...
        elif self.dset_name in self.INFLUX_SYNTH_FORMAT:
            assert f"{split}_h5" in conf, f"Missing {split}_h5 in conf"
            self.datapoints = load_h5_influx_synth_format(
                Path(self.conf.get(f"{split}_h5")), self.img_dir
            )
        else:
        ...
        self.produce_rays = conf.produce_rays
    ...
    def _read_image(
        ...
        image = load_image(path, self.conf.grayscale, return_tensor=False).numpy()

        map_x, map_y = self.get_precomputed_dist_map(datapoint)
        ...
        if self.produce_rays:
            data_dict["rays"] = rays
            data_dict["rays_mask"] = valid

        return data_dict

    def get_precomputed_dist_map(
        self, datapoint: DataPoint
    ) -> tuple[np.ndarray | None, np.ndarray | None]:
        """Load precomputed distortion maps from H5 when available."""
        if self._dist_map_h5_file is None:
            self._dist_map_h5_file = h5py.File(datapoint.map_h5_path, "r", libver="latest")

        group = self._dist_map_h5_file[datapoint.map_group_name]
        map_x = group["dist_map_x"][:].astype(np.float32, copy=False)
        map_y = group["dist_map_y"][:].astype(np.float32, copy=False)
        return map_x, map_y
