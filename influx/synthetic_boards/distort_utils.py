...
def get_distorted_image(intrinsics, distortion, image):
    ...
    x_distorted = x_distorted.astype(np.float64)
    y_distorted = y_distorted.astype(np.float64)
    x_distorted += 0.5
    y_distorted += 0.5
    x_distorted = x_distorted.flatten()
    y_distorted = y_distorted.flatten()
    distorted_coords = np.hstack((x_distorted[..., None], y_distorted[..., None]))

    x, y = get_undistorted_coords(intrinsics, distortion, distorted_coords)

    x = x.reshape(h, w)
    y = y.reshape(h, w)

    # Remap the image; note that remap uses (0, 0) as pixel center, so we shift by 0.5 pixels again
    distorted_image = cv2.remap(image, x.astype(np.float32) - 0.5, y.astype(np.float32) - 0.5, cv2.INTER_LINEAR)
