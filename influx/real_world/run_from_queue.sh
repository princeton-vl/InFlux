...
QUEUE_FILE=$(python3 -c "import yaml; print(yaml.safe_load(open('$(dirname "$0")/../config.yaml'))['real_world']['QUEUE_FILE'])")
COMPLETED_FILE=$(python3 -c "import yaml; print(yaml.safe_load(open('$(dirname "$0")/../config.yaml'))['real_world']['COMPLETED_FILE'])")
ONHOLD_FILE=$(python3 -c "import yaml; print(yaml.safe_load(open('$(dirname "$0")/../config.yaml'))['real_world']['ONHOLD_FILE'])")
SKIP_IF_FAIL_FLAG=$(python3 -c "import yaml; print(yaml.safe_load(open('$(dirname "$0")/../config.yaml'))['real_world']['SKIP_IF_FAIL_FLAG'])")
